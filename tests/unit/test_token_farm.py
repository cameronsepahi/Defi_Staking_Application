...
def test_set_price_feed_contract():
    # Since this is one of our unit tests, we want to make sure we are on a local networ
    if network.show_active() not in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        pytest.skip("Only for local testing")
    account = get_account()
    non_owner = get_account(index=1)  # Gives us a different account
    ...
    # Testing that someone else cannot run contract besides the owner of the address, expecting to revert
...
def test_issue_tokens(amount_staked):
    ...
    # Arrange
    # We are staking 1 dapp_token, which is equal in price to 1 ETH, so we should
    # get 2,000 dapp tokens in reward
    assert (
